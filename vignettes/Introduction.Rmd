...
(See the Loggers vignette, available via `vignette("Loggers", package = "magenta")`, for more details on the logging output.)
...
*Plot output omitted here; knit the vignette locally to reproduce the figure.*
...
The above 4 lists are sufficient for restarting a simulation and can thus be used to save the model state to file and then be reloaded. See the Loggers vignette (`vignette("Loggers", package = "magenta")`) for more information on what these lists contain, and the Advanced features vignette (`vignette("AdvancedFeatures", package = "magenta")`) for practical advice on saving and reloading model runs.
...
`magenta`, however, has been developed to also simulate drug resistance and consider more nuanced spatial set ups. The dedicated Resistance vignette (`vignette("Resistance", package = "magenta")`) provides further worked examples. Exploratory features such as vector adaptation and non-malarial fevers are covered in the Advanced features vignette (`vignette("AdvancedFeatures", package = "magenta")`).
