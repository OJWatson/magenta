...
The model is written in C++/Rcpp and interfaced with R, incorporating both the deterministic version of the Imperial College Transmission model developed by Joel Hellewell and Hannah Slater, which helps initialise the model near equilibrium. In addition it is used to quickly the mosquito feeding behaviour, which is passed to `magenta` in order to increase its speed.    
...
- **`Loggers`**: brief overview of the logging information returned by `magenta` (TO DO)
